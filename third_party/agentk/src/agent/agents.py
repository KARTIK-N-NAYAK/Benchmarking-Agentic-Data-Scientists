...
class LLMAgent(Agent):
    def __init__(
            ...
            continue_after_end_of_file: bool = True,
            **kwargs,
    ):
        """
        Args:
        ...
        # self.llm: LanguageBackend = llm(
        #     logger=self.logger,
        #     responses_from_file=self.llm_answers,
        #     continue_after_end_of_file=continue_after_end_of_file
        # )
        
        if isinstance(llm, LanguageBackend):
            self.llm = llm
        else:
            self.llm: LanguageBackend = llm(
                logger=self.logger,
    ...
    def chat_completion(self, llm: LanguageBackend, messages: list[dict[str, str]], parse_func: Callable,
        ...
        response, runtime = llm._chat_completion(messages=messages, parse_func=parse_func, **kwargs)
        chat_completion_time = time.time() - start_time
        # `chat_completion_time` can be different than `runtime` if there have been retrials for chat completion
        retrial_time = chat_completion_time - runtime
        self.memory.store(content=retrial_time, tags=MemKey.RETRIAL_CHAT_COMPLETION_TIME)

        self.set_human_mode(human_mode=llm.human_mode)
        return response

    @staticmethod
    def get_llm_answers(read_answer_from_file_path: str | Path | list[str] | None) -> list[str] | None:
        if isinstance(read_answer_from_file_path, str):
            read_answer_from_file_path = Path(read_answer_from_file_path)
            
        if read_answer_from_file_path is None:
            return None
        elif isinstance(read_answer_from_file_path, Path):
            if read_answer_from_file_path.suffix == '.txt':
                raw_answer_content = read_answer_from_file_path.read_text()
                reg_str = "<ANS>(.*?)</ANS>"
                answers = re.findall(reg_str, raw_answer_content, re.DOTALL)
                return answers

            else:
                return get_responses_from_file(log_path=str(read_answer_from_file_path))
    ...
    def safe_parsing_chat_completion(self, ask_template: str, parse_func: Callable, format_error_message: str,
                                     prompt_kwargs: dict[str, Any] = None, max_retries: int = 5,
        ...
        self.llm.history[-1]["parsed_response"] = response
        return response
...
class ThinkAndCodeLLMAgent(MultiLLMAgent):
    ...
    def __init__(
        ...
        super().__init__(
            llm=llm,
            specialized_llms={"code": code_llm if code_llm else llm},
            embedding=embedding,
            memory=memory,
            prompt_builder=prompt_builder,
            on_init_flow=on_init_flow,
...
def safe_parsing_chat_completion(
    ...
    Args:
        human_takeover: whether human should provide the output if LLM keeps getting parsing errors
        predefined_response: if set, this response content will be used instead of querrying an LLM
    """
    response = None
    _count = 0
    format_err_msg = None
        ...
        try:
            ...
            if specialized_llm_name is not None:
                ...
                assert hasattr(agent, 'specialized_llms'), \
                    "Trying to query a specialized llm but agent doesn't have any."
                ...
                response = agent.chat_completion(
                    llm=llm, messages=llm_input, parse_func=parse_func, predefined_response=predefined_response,
                    **kwargs
                )
            else:
                llm = agent.llm
