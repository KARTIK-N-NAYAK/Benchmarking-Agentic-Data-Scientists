...
class ConsiderAction(Command):
    ...
    def func(self, agent, avail_actions_mem_key, output_mem_key, ask_template):
        if agent.task.action_space == ActionSpace.DISCRETE:
            assert agent.memory.retrieve({avail_actions_mem_key: 1.0})
        ...
        if self.append:
            ...
            prev_actions = None if prev_actions is None or prev_actions is None else prev_actions
...
class ChooseRandomAction(Command):
    """Performs a random action in an environment with a discrete ActionSpace"""

    name: str = "random_act"
    description: str = "Randomly choose an action that will be sent to the environment."

    input_keys: dict[str, MemKey] = {"avail_actions_mem_key": MemKey.AVAILABLE_ACTIONS}
    output_keys: dict[str, MemKey] = {"planned_action_mem_key": MemKey.NEXT_PLANNED_ACTION}

    seed: int | None = None
    weights: dict[Any, float] | None = None

    # Runtime-only attribute (NOT part of the pydantic model)
    ...
    def func(self, agent, avail_actions_mem_key, planned_action_mem_key):
        assert agent.task.action_space == ActionSpace.DISCRETE, (
            "RandomAction can only be used with discrete action spaces"
        )
        ...
        p = None
        if self.weights:
            p = np.array([self.weights.get(o, 0) for o in options])
            p = p / p.sum() if p.sum() > 0 else None

        chosen_action = self.rng.choice(options, p=p)
        agent.memory.store(chosen_action, {planned_action_mem_key})
