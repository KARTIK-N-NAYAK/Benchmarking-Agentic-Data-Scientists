...
def query(
    ...
    **model_kwargs,
    ...
    # Determine API to use - always use chat completions if custom base URL is set
    model_name = filtered_kwargs.get("model", "")
    is_openai_model = re.match(r"^(gpt-|o\d-|codex-mini-latest$)", model_name)
    use_custom_base_url = os.getenv("OPENAI_BASE_URL") is not None or OPENAI_BASE_URL != "https://api.openai.com/v1"
    
    # Use chat completions API for custom base URLs, responses API only for official OpenAI
    use_chat_api = use_custom_base_url or not is_openai_model

    # Only convert max_tokens for responses API, not for chat completions
    if not use_chat_api and "max_tokens" in filtered_kwargs:
    ...
    if use_chat_api:
        ...
        # OpenAI responses API (for official OpenAI models only)
        messages = opt_messages_to_list(system_message, user_message)
