...
# for Internal LLM support
from .mllab_chat import AssistantMLLABChatOpenAI, create_local_chat, get_local_models
...
class ChatLLMFactory:
    ...
    def get_valid_models(cls, provider):
        ...
        elif provider == "mllab":               # for Internal LLM support
    ...
    def get_valid_providers(cls):
        return ["azure", "openai", "bedrock", "anthropic", "sagemaker", "mllab"]  # for Internal LLM support
    ...
    def get_chat_model(cls, config: DictConfig, session_name: str) -> Union[
        ...
        AssistantMLLABChatOpenAI,  # for Internal LLM support
    ]:
        ...
        if provider == "openai":
            ...
            return create_local_chat(config, session_name)
        else:
            raise ValueError(f"Unsupported provider: {provider}")
