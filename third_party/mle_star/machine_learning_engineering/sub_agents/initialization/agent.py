...
def get_model_candidates(
    ...
    try:
        ...
        print("[[get_model_candidates123123]]Model candidates response text:", response_text)
        start_idx, end_idx = response_text.find("["), response_text.rfind("]")+1
        result = response_text[start_idx:end_idx]
        print("[[get_model_candidates123123]]Extracted model candidates string:", result)
...
def get_task_summary(
    ...
    print("[[get_task_summary123123]]Generated task summary:", task_summary)
...
def check_model_eval_finish(
    ...
    print("[[check_model_eval_finish123123]]Model evaluation execution result:", result_dict)
    if result_dict and result_dict.get("returncode") == 0:
        return llm_response_module.LlmResponse()
...
def check_merger_finish(
    ...
    print("[[check_merger_finish123123]]Code integration execution result:", result_dict)
...
def skip_data_use_check(
    ...
    print("[[skip_data_use_check123123]]Training code execution result:", result_dict)
...
def rank_candidate_solutions(
    ...
    print("[[rank_candidate_solutions123123]Ranking candidate solutions...")
    workspace_dir = callback_context.state.get("workspace_dir", "")
    print("123123Workspace directory:", workspace_dir)
    task_name = callback_context.state.get("task_name", "")
    print("123123Task name:", task_name)
    task_id = callback_context.agent_name.split("_")[-1]
    print("Task ID:", task_id)
    run_cwd = os.path.join(workspace_dir, task_name, task_id)
    print("[[rank_candidate_solutions123123]]Run current working directory:", run_cwd)
    num_model_candidates = callback_context.state.get("num_model_candidates", 2)
    print("[[rank_candidate_solutions123123]]Number of model candidates:", num_model_candidates)
    performance_results = []
    for k in range(num_model_candidates):
        print("[[rank_candidate_solutions123123]]Processing model candidate:", k + 1)
        model_id = k + 1
        ...
        print(f"[[rank_candidate_solutions123123]]Init code execution result for model {model_id}:", init_code_exec_result)
        if init_code_exec_result:
            print(f"[[rank_candidate_solutions123123]]Score for model {model_id}:", init_code_exec_result.get("score", 0.0))
    ...
    if not performance_results:
        print(f"Warning: All models for task {task_id} failed. Assigning fallback penalty.")
        penalty = float('inf') if callback_context.state.get("lower", True) else float('-inf')
        performance_results.append((penalty, "print('All models failed')", {"score": penalty}))
    print("[[rank_candidate_solutions123123]]Sorted performance results:", performance_results)
    best_score = performance_results[0][0]
    print("[[rank_candidate_solutions123123]]Best score among candidate solutions:", best_score)
...
def select_best_solution(
    ...
    print("[[select_best_solution123123]]Output file path for best solution:", output_filepath)
...
def update_merger_states(
    ...
    print("[[update_merger_states123123]]Merged code execution result:", result_dict)
    if not result_dict or "score" not in result_dict:
        print(f"[[update_merger_states123123]] Warning: No valid score returned for {callback_context.agent_name}. Skipping state update.")
        return None
...
def get_merger_agent_instruction(
    ...
    print("[[update_merger_states123123]]Performance results:", performance_results)
